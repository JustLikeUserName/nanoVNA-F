...
void MainWindow::onVnaReadyRead()
{
    ...
    if(retMsg.contains("ch>") && retMsg.contains("scan"))
    {
        ...
        if(pos2>pos1) //pos1至少是0
        {
            ...
            double freq[N];//定义频率参数
            double s11_real[N];//定义s11实部
            double s11_im[N];//定义s11虚部
            double s21_real[N];//定义s11实部
            double s21_im[N];//定义s11虚部
            double s11Amplitude[N];
            double s21Amplitude[N];
            ...
            if(vnaStatus == ReadStatus::S11AndS21)
            {
                for(int i=1;i<N+1;++i)
                {
                    QStringList list2 = dataList.at(i).split(' ');//是空格分隔，还是'\t'
                    if(list2.length()>=5)
                    {
...
void MainWindow::on_s11SeriesHovered(const QPointF &point, bool state)
{
    ...
    if (state)
    {
       pS11Callout->setText(QString("S11: %1dB/Freq:%2Hz").arg(point.y()).arg(point.x()));
...
void MainWindow::on_s21SeriesHovered(const QPointF &point, bool state)
{
    ...
    if (state)
    {
       pS21Callout->setText(QString("S21: %1dB/Freq:%2Hz").arg(point.y()).arg(point.x()));
